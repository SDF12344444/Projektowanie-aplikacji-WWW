...
function PokazKategorie($conn) {
    ...
    echo '</table>';
    echo '<br><a href="?action=adddd">Dodaj nową kategorię</a>';
}
...
if (isset($_GET['action'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // Obsługa nadrzędnej kategorii
        $parent_name = $_POST['parent_name'];

        // Sprawdź, czy podana kategoria nadrzędna istnieje w bazie
        if (!empty($parent_name)) {
            ...
            if ($result->num_rows > 0) {
                // Kategoria nadrzędna istnieje
                $row = $result->fetch_assoc();
                $parent_id = $row['id'];
            } else {
                // Jeśli kategoria nadrzędna nie istnieje, ustaw brak
                $parent_id = 0;
            }
        } else {
            $parent_id = 0; // Domyślnie brak kategorii nadrzędnej
        }

        // Dodaj kategorię
        DodajKategorie($conn, $_POST['name'], $parent_id);
    ...
    echo '<label for="parent_name">Kategoria nadrzędna (wpisz nazwę):</label>';
    echo '<input type="text" name="parent_name" id="parent_name" placeholder="Brak" value=""><br><br>';
...
if (isset($_POST['show_tree'])) {
    ...
    GenerujDrzewoKategorii($conn); // Wywołanie funkcji generującej drzewo
} else {
    echo '<form method="post">';
    echo '<button type="submit" name="show_tree">Pokaż drzewo kategorii</button>';
    echo '</form>';
}
